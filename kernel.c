...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
	...
	-- Checking for any free processes 
	check = freeList.IsEmpty()
	if check == true
	...
	-- Grabbing a process to return from the freelist
	pcb = freeList.Remove()
	pcb.status = ACTIVE
	...
	-- Checking to see if the condition needs to be woken up
	check = freeList.IsEmpty()
	
	-- Adding p to the freeList
	freeList.AddToEnd(p)
	p.status = FREE

	if check == true
	 * mu = processManagerLock
	 aProcessBecameFree.Signal(mu)
	endIf
...
-----------------------------  InitFirstProcess -----------------------------
  ...
  function InitFirstProcess ()
    --
    -- This routine is called to initialize my first process. It will get a
    -- new thread from the thread manager.  
    --
...
---------------------------- StartUserProcess -------------------------------
  ...
  function StartUserProcess
    ...
    --
    -- This routine is meant to begin the user process.  It allocates a new
	...
	OF = fileManager.Open ("TestProgram1")
	i = OF.LoadExecutable (myPCB.addrSpace)
	fileManager.Close(OF)

	initSystemStackTop = & currentThread.systemStack[SYSTEM_STACK_SIZE]

	p = SetInterruptsTo (DISABLED)


        myPCB.addrSpace.SetFrameAddr(initSystemStackTop)
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
	...
	var
	    i, f: int
	-- Getting access to frameManager
	frameManagerLock.Lock()

	--Getting the correct amount of frames necessary
	while numberFreeFrames < numFramesNeeded
	  newFramesAvailable.Wait (&frameManagerLock)
	endWhile

	--Allocating frames and storing their addresses

	for i = 0 to numFramesNeeded - 1
	  f = framesInUse.FindZeroAndSet()
	  aPageTable.SetFrameAddr(i, f)
	endFor

	--Adjusting the number of free frames
	numberFreeFrames = numberFreeFrames - numFramesNeeded
	
	--setting numberOfPages
	...
	-- Setting all frame addresses back to zero
	...
	for i = 0 to  num - 1 
	  ...
	  framesInUse.ClearBit(frameAddr)
	endFor

	--Adjusting number of free frames
	numberFreeFrames = numberFreeFrames + num

	-- Waking up any other threads it needs to
